...
**2026-07-19 — site-wide character corruption (mojibake) introduced by a bulk edit, repaired across all 46 affected pages.**
The 2026-07-18 commit that re-stamped the nav cache-bust version across every published page (`2b8790a`) was executed by tooling that read the UTF-8 files with the Windows default cp1252 codec and re-saved them, double-encoding every non-ASCII character — em dashes rendered as `â€”`, curly quotes and middle dots similarly garbled, on every page the commit touched. The old host's charset handling masked the damage; the move to GitHub Pages (2026-07-19), which serves the charset the pages declare, exposed it. Repaired by mechanically reversing the mis-encoding in all 46 files (no wording changed — the diffs are character-for-character), verified by a strict-UTF-8 and garble-signature sweep of the whole directory. `tools/check_published.py` — which passed the corrupted commit — now fails on invalid UTF-8 and on the mojibake signature itself, tested against the corrupted snapshot. Settled by: git history bisection (the commit before the re-stamp is clean; every file it touched came out garbled), and the repaired pages read back byte-for-byte.
