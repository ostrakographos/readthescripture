...
It is not the workbench. Drafts, rejected pieces, research notes, working transcripts, and the editorial back-and-forth that precedes publication stay private — the same boundary any publication draws between its archive and its pressroom. Nothing here is redacted from a private original; public documents are authored for this record. The boundary is stated rather than hidden, and the record grows from its start date (2026-07-14) forward; `CORRECTIONS.md` additionally backfills the significant corrections that predate it.
